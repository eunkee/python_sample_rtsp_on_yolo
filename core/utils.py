...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    for i in range(num_boxes[0]):
        ...
        if show_label:
            ...
            cv2.rectangle(image, c1, (int(np.float32(c4[0])), int(np.float32(c4[1]))), bbox_color, -1) #filled

            cv2.putText(image, bbox_mess, (c1[0], int(np.float32(c1[1] + t_size[1]))), cv2.FONT_HERSHEY_SIMPLEX,
